...
const long R_IOUT_UPPER = 560000L;
...
void setDesired(float voltage){
  ...
  desired = voltage;
  pwm_out = PWM_MIN + ((correctDesiredVoltage(desired) - V_MIN) * (PWM_MAX - PWM_MIN)) / (V_MAX - V_MIN);
...
float correctCurrent(float measuredCurrent, float actualVoltage){
  /*
   * Adjusts the measured current according to a predetermined polynomial fit.
   * 
   */
   ...
   return measuredCurrent+((A*desired*desired)+(B*desired)+C);
}

float correctVoltage(float measuredVoltage){
  /*
   * Adjusts the measured voltage according to a constant increase/decrease.
   */

   float constant = -0.07;
   return measuredVoltage+constant;
}

float correctDesiredVoltage(float desiredVoltage){
  /*
   * Adjusts the desired voltage by a constant amount
   */

   float constant = 0.13;
   return desiredVoltage+constant;
}
...
void printStatus(){
  ...
  String msg = "DESIRED: "+ String(desired)+ "V ACTUAL: "+ String(correctVoltage(getActualVoltage()))+ "V DRAW: "+ String(getCurrent())+ "mA CORRECTED DRAW: "+ String(correctCurrent(getCurrent(), correctVoltage(getActualVoltage())));
  Serial.println(msg);
